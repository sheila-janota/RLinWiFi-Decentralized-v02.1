...
class Teacher:
    ...
    def eval(self, agents, simTime, stepTime, history_length, tags=None, parameters=None, experiment=None):
        ...
        try:
           logger.begin_logging(1, steps_per_ep, agents.noise.sigma, agents.noise.theta, stepTime)
    ...
    def train(self, agents, EPISODE_COUNT, simTime, stepTime, history_length, send_logs=True, experimental=True, tags=None, parameters=None, experiment=None):
        ...
        try:
            logger.begin_logging(EPISODE_COUNT, steps_per_ep, agents.noise.sigma, agents.noise.theta, stepTime)
        except  AttributeError:
        ...
        for i in range(EPISODE_COUNT):
            ...
            obs = self.env.reset()
            obs = self.preprocess(np.reshape(obs, (-1, len(self.env.envs), obs_dim)))
            #print("train method -obs:", obs)
            #print("train method -obs_dim:", obs_dim)
            #print ("Print Obs Shape: ", obs.shape)
            ...
            with tqdm.trange(steps_per_ep) as t:
                for step in t:
                    self.debug = obs
                    #actions_list = []
                    #actions_list_dim = [self.stations]                
                    actions_list = np.empty(shape=(0,), dtype=np.int32) 
                               # = np.empty(shape=(self.stations,), dtype=np.int32) 
                               # =  np.array(self.stations, dtype=np.int32)
                    ...
                    #for i, agent in enumerate(agents):                    
                        ...
                        #print("Train - self.actions-shape: ", self.actions.shape())
                        #print("Train - self.actions: ", self.actions, "Train - self.actions-size: ", len(self.actions))
                        #print("Train - self.actions-shape: ", self.actions)
                        #actions_list.append(self.actions)
                        actions_list = np.append(actions_list, self.actions)
                        #actions_list = np.concatenate((actions_list, self.actions), axis=0)
                        #print("Train - actions_list", actions_list)
                        #print("actions-shape", len(self.actions))
                    #actions_list = actions_list[1:]
		    
                    #print("actions_list: ", actions_list)
                    #print("Obs-np-array: ", np.array(obs, dtype=np.float32))
                    #print("Train - actions_list do agent que vai no step", actions_list[-1])
                    #print("These is the actions ",actions_list, actions_list[0], actions_list[1])
                    next_obs, reward, done, info = self.env.step(actions_list)
                    #next_obs, reward, done, info = self.env.step(actions_list[i])
                    #print("Train - actions_list", actions_list[-1])
